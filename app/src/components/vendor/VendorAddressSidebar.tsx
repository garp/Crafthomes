...
import type { TVendorAddressSidebarProps } from '../../types/vendor';
...
import { vendorAddressSchema } from '../../utils/addressValidation';

export default function VendorAddressSidebar({ isOpen, onClose }: TVendorAddressSidebarProps) {
  const field = (name: string) => `address.${name}`;
  ...
  const {
    ...
    errors,
    touched,
  } = useFormikContext<TAddVendorFormData>();
  ...
  const address: TAddress = (formValues.address as unknown as TAddress) || {};
  const addressErrors = (errors as any)?.address || {};
  const addressTouched = (touched as any)?.address || {};
        ...
        <Button
          ...
          onClick={async () => {
            ...
            try {
              ...
              await vendorAddressSchema.validate(normalizedAddress, { abortEarly: false });
